...
class Badge(object):
    ...
    This class can be instantiated to generate many badges with the same format::
    ...
    def __init__(self, *args, **kwargs):
        ...
        Most configuration options are simply CSS settings. Please visit the w3c site for
        ...
        label_background -- background color for the label (left) part (CSS "background")
        label_text_color -- text color for the label (left) part (CSS "text-color")
        font_family -- font to use in the badge (CSS "font-family")
        padding -- amount of space between the border and the badge (CSS "padding")
    ...
    def _parse_args(cls, args, kwargs):
        ...
        """
        if len(args) > 2:
            raise ValueError('a maximum of 2 optional argument may be given ({} were given.)'
                             ''.format(len(args)))
    ...
    def _get_value_background(cls, config):
        ...
        if 'thresholds' in config:
            return config['thresholds'].get(config['value'], config['value_background'])
